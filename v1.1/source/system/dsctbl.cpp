...
void g_mes1_s( int x, int y, char *font) {
	int *p;
	char d ;
...
void panic3(void){
	io_cli();
	for(int x=1024*100; x<sinfo->scrnx*sinfo->scrnx; x+=3)
		sinfo->vram[x]=0xff00;
...
void panic_CPU(int CS, int EIP){
	...
	for(int y=0; y<40; y++)
		for(int x=0; x<140; x++)
			sinfo->vram[y*sinfo->scrnx+x]=0xff0000;
	char x[70];
	sprintf(x,"Kernel Panic!");
	g_mes_s(10,10,x);
...
//例外発生時に呼び出すパニック関数（CSもEIPも違う気がする。インテルの資料の読み間違い？）
void panic_CPU3(int CS, int EIP){
	io_cli();
	for(int x=1024*100; x<sinfo->scrnx*sinfo->scrnx; x+=3)
		sinfo->vram[x]=0xff00;
	...
	for(int y=100; y<140; y++)
		for(int x=0; x<200; x++)
			sinfo->vram[y*sinfo->scrnx+x]=0xff0000;
	char x[70];
	sprintf(x,"Interrupt Number:0x0D");
	g_mes_s(10,110,x);
/*	sprintf(x,"Current TASK GDT Number:%d",taskctl.current->GDT_num);
	g_mes_s(10,30,x);
*/
	for(;;)
		asm("hlt");
...
void init_gdtidt(void)
{
	...
	for (char cnt=0x08; cnt<0x20; cnt++)
		set_gatedesc(cnt, (int) panic_CPU, 2 * 8, AR_INTGATE32);
	set_gatedesc(0x0D, (int) panic_CPU3, 2 * 8, AR_INTGATE32);
	...
	set_gatedesc(0x40, (int) asm_inthandler40, 2 * 8, AR_INTGATE32);
	set_gatedesc(0x41, (int) asm_inthandler41, 2 * 8, AR_INTGATE32);
