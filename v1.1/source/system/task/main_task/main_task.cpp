...
void main_task(void) {
	...
	//タスク追加
	taskctl.Create(timer_task);	//タイマ処理
	taskctl.Create(aqua_task);	//AquaGUIテスト
	taskctl.Create(sample_03_basic);	//d3moduleテスト


	char s[20];
	taskctl.SetInput(MOUSE_ID_S, new Caller(mouse_signal)) ;
	enable_mouse();					// マウス有効化
	KEYBOARD keyboard;
	taskctl.SetInput(KEYBOARD_ID, new Caller(keyboard, &KEYBOARD::FuncSignalProcess)) ;
...
void mouse_signal(UCHAR dat) {
	...
	if (mouse_decode()) {
		...
		g_mes(window,6,85,"mouse input:");
		g_mes(window,6,105,s);
		sprintf(s, "mouse pos: (%3d, %3d)", mouse->vx, mouse->vy);
		g_mes(window,6,125,s);
