...
CONSOLE::CONSOLE(const short windowPosX, const short windowPosY, const short windowSizeX, const short windowSizeY, const UINT bufX, const UINT bufY){
	...
	_charOfCurrent=0;
	_typeString("OASIS v1.1.0") ;
	_newline();
	_typeString("Created by OASIS Developer Team on 2011/01") ;
	_newline();
	_typeString("License is GPL v2 only") ;
	_newline();
	_newline();
	_typeString("Type 'help' and display information.") ;
	_newline();
	_newline();
	_typeString("> ") ;
	...
	taskctl.SetInput(KEY_ID,&caller_key) ;
...
void CONSOLE::lootin(UCHAR key) {
	...
	switch (key) {
		case 8 :		// バックスペース
			...
			g_boxf(_window, (_charOfCurrent%_bufferX)*8, ((_charOfCurrent-_charOfCorner)/_bufferX)*16, 8, 16, 0xFFFFFF );
...
void CONSOLE::_typeChar(void) {
	...
	g_mes( _window, (_charOfCurrent%_bufferX)*8, ((_charOfCurrent-_charOfCorner)/_bufferX)*16, &_buffer[_charOfCurrent] );
	windowctl.refresh(_window, (_charOfCurrent%_bufferX)*8, ((_charOfCurrent-_charOfCorner)/_bufferX)*16, 8, 16) ;
	_charOfCurrent++;
...
void CONSOLE::_scroll(void) {
	...
	g_boxf(_window, 0, _window->ysize-16, _window->xsize, 16, 0xFFFFFF );
...
void CONSOLE::cursor_func( UCHAR tmp ) {
	...
	g_boxf( _window, (_charOfCurrent%_bufferX)*8, ((_charOfCurrent-_charOfCorner)/_bufferX)*16+15, 8, 1, 0xFFFFFF&_cursor );
